...
$Launcher = Join-Path $Root "dist\Open-Noises-Online.bat"
...
if (-not (Test-Path -LiteralPath $Launcher)) {
	throw "No existe el launcher. Ejecuta primero: .\scripts\build.ps1"
}
$ZipName = "Noises-Online-v$Ver-Win32-x64.zip"
$ZipPath = Join-Path $Root $ZipName
if (Test-Path -LiteralPath $ZipPath) { Remove-Item -LiteralPath $ZipPath -Force }
Compress-Archive -Path $Launcher, $Source -DestinationPath $ZipPath -CompressionLevel Optimal
